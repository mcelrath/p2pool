...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    def get_current_scaled_txouts(scale, trunc=0):
        ...
        results = dict((script, value*scale//total) for script, value in list(txouts.items()))
        if trunc > 0:
            total_random = 0
            random_set = set()
        ...
        if sum(results.values()) < int(scale):
            results[math.weighted_choice(iter(list(results.items())))] += int(scale) - sum(results.values())
        return results
    ...
    def get_patron_sendmany(total=None, trunc='0.01'):
        ...
        return json.dumps(dict(
            ...
            for script, value in list(get_current_scaled_txouts(total, trunc).items())
    ...
    def get_local_stats():
        ...
        return dict(
            ...
            peers=dict(
                incoming=sum(1 for peer in list(node.p2p_node.peers.values()) if peer.incoming),
                outgoing=sum(1 for peer in list(node.p2p_node.peers.values()) if not peer.incoming),
            ),
            shares=dict(
                total=shares,
    ...
    web_root.putChild('user_stales', WebInterface(lambda: dict((bitcoin_data.pubkey_hash_to_address(ph, node.net.PARENT), prop) for ph, prop in
        list(p2pool_data.get_user_stale_props(node.tracker, node.best_share_var.value, node.tracker.get_height(node.best_share_var.value)).items()))))
    web_root.putChild('fee', WebInterface(lambda: wb.worker_fee))
    web_root.putChild('current_payouts', WebInterface(lambda: dict((bitcoin_data.script2_to_address(script, node.net.PARENT), value/1e8) for script, value in list(node.get_current_txouts().items()))))
    web_root.putChild('patron_sendmany', WebInterface(get_patron_sendmany, 'text/plain'))
    web_root.putChild('global_stats', WebInterface(get_global_stats))
    web_root.putChild('local_stats', WebInterface(get_local_stats))
    web_root.putChild('peer_addresses', WebInterface(lambda: ' '.join('%s%s' % (peer.transport.getPeer().host, ':'+str(peer.transport.getPeer().port) if peer.transport.getPeer().port != node.net.P2P_PORT else '') for peer in list(node.p2p_node.peers.values()))))
    web_root.putChild('peer_txpool_sizes', WebInterface(lambda: dict(('%s:%i' % (peer.transport.getPeer().host, peer.transport.getPeer().port), peer.remembered_txs_size) for peer in list(node.p2p_node.peers.values()))))
    ...
    web_root.putChild('peer_versions', WebInterface(lambda: dict(('%s:%i' % peer.addr, peer.other_sub_version) for peer in list(node.p2p_node.peers.values()))))
    ...
    hd = graph.HistoryDatabase.from_obj({
        ...
        'local_share_hash_rates': graph.DataStreamDescription(dataview_descriptions, is_gauge=False,
            ...
            default_func=graph.make_multivalue_migrator(dict(good='local_share_hash_rate', dead='local_dead_share_hash_rate', orphan='local_orphan_share_hash_rate'),
                post_func=lambda bins: [dict((k, (v[0] - (sum(bin.get(rem_k, (0, 0))[0] for rem_k in ['dead', 'orphan']) if k == 'good' else 0), v[1])) for k, v in list(bin.items())) for bin in bins])),
    ...
    def add_point():
        ...
        miner_hash_rates, miner_dead_hash_rates = wb.get_local_rates()
        current_txouts_by_address = dict((bitcoin_data.script2_to_address(script, node.net.PARENT), amount) for script, amount in list(current_txouts.items()))
        ...
        hd.datastreams['desired_version_rates'].add_datum(t, dict((str(k), v/vs_total*pool_total) for k, v in list(vs.items())))
        try:
