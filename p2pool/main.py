...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        bitcoind = jsonrpc.HTTPProxy(bytes(url, "utf-8"), dict(Authorization='Basic ' + str(base64.b64encode(bytes((args.bitcoind_rpc_username + ':' + args.bitcoind_rpc_password), 'utf-8')),'utf-8')), timeout=30)
        ...
        node = p2pool_node.Node(factory, bitcoind, list(shares.values()), known_verified, net)
        yield node.start()
    ...
    if args.bitcoind_rpc_password is None:
        ...
        for line in conf.splitlines(True):
            if '#' in line.decode("utf-8"):
                line = line[:line.index('#'.encode("utf-8"))]
