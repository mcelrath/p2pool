...
DONATION_SCRIPT = b'4104ffd03de44a6e11b9917f3a29f9443283d9871c9d743ef30d5eddcd37094b64d1b3d8090496b53256786bf5c82932ec23c3b74d9f05a6f95a8b5529352656664bac'
...
class BaseShare(object):
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None, segwit_data=None):
        ...
        amounts = dict((script, share_data['subsidy']*(199*weight)//(200*total_weight)) for script, weight in list(weights.items())) # 99.5% goes according to weights prior to this share
        ...
        if sum(amounts.values()) != share_data['subsidy'] or any(x < 0 for x in list(amounts.values())):
            raise ValueError()

        dests = sorted(iter(list(amounts.keys())), key=lambda script: (script == DONATION_SCRIPT, amounts[script], script))[-4000:] # block length limit, unlikely to ever be hit
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def apply_delta(self, xxx_todo_changeme3, xxx_todo_changeme4, xxx_todo_changeme5):
        ...
        if total_weight1 + total_weight2 > desired_weight and share_count2 == 1:
            ...
            script, = iter(list(weights2.keys()))
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    if rates:
        ...
        res = dict((k, v/dt) for k, v in list(res.items()))
    return res
...
def get_user_stale_props(tracker, share_hash, lookbehind):
    ...
    return dict((pubkey_hash, stale/total) for pubkey_hash, (stale, total) in list(res.items()))
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    ...
    res = dict((script, subsidy*weight//total_weight) for script, weight in list(weights.items()))
...
class ShareStore(object):
    def __init__(self, prefix, net, share_cb, verified_hash_cb):
        ...
        self.known_desired = dict((k, (set(a), set(b))) for k, (a, b) in list(known.items()))
    ...
    def add_share(self, share):
        for filename, (share_hashes, verified_hashes) in list(self.known.items()):
    ...
    def forget_share(self, share_hash):
        for filename, (share_hashes, verified_hashes) in list(self.known_desired.items()):
